#include "Graphics.h"
#include "Globals.h"
#include "rlights.h" // Ensure this includes Light, MAX_LIGHTS, and CreateLight
...
Graphics::Graphics(Image obstacleImg) : obstacleImg_(ImageCopy(obstacleImg))
{
    ...
    groundModelpos_ = Vector3{-globals.WORLD_SZ / 2.f, 0.0f, -globals.WORLD_SZ / 2.f}; // Define model position
...
void Graphics::draw()
{
    ...
    // Draw Ground
    DrawModel(groundModel_, groundModelpos_, 1., WHITE);
    // Add code to draw other objects, such as robots and tasks
    EndMode3D();
    EndDrawing();
}
